...
public class UserService
{
    ...
    {
        if (await CheckIfEmailExists(email) == false) return null;

        var user = await _userRepository.GetUserByEmailAsync(email);

        return user != null ? _mapper.Map<UserResponseDto>(user) : null;
    }
    public async Task<string?> GetUserHashedPasswordByUsernameAsync(string email)
    {
        return await _userRepository.GetUserPasswordHashByUsernameAsync(email);
    }
